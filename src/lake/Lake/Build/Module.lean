...
public def Module.cacheOutputHashes (mod : Module) : IO PUnit := do
  ...
  if (← mod.rsFile.pathExists) then
    cacheFileHash mod.rsFile
...
: EIO String ModuleOutputArtifacts := do
  let arts : ModuleOutputArtifacts := {
    ...
    bc? := none
    rs? := ← descrs.rs?.mapM cache.getArtifact
...
: JobM ModuleOutputArtifacts := do
  return {
    ...
    rs? := ← cacheIf? (← mod.rsFile.pathExists) mod.rsFile "rs"
    vm? := ← cacheIf? (← mod.vmFile.pathExists) mod.vmFile "vm"
  }
where
...
private def Module.restoreAllArtifacts (mod : Module) (cached : ModuleOutputArtifacts) : JobM ModuleOutputArtifacts := do
  return {cached with
    ...
    rs? := ← restoreSome mod.rsFile cached.rs?
...
private def Module.mkArtifacts (mod : Module) (srcFile : FilePath) (isModule : Bool) : ModuleArtifacts where
  ...
  rs? := if mod.backend == .rust then some mod.rsFile else none
...
private def Module.computeArtifacts (mod : Module) (isModule : Bool) : FetchM ModuleOutputArtifacts :=
  return {
    ...
    rs? := ← computeIf (mod.backend == .rust) mod.rsFile "rs"
...
/-- The `ModuleFacetConfig` for the builtin `rsFacet`. -/
public def Module.rsFacetConfig : ModuleFacetConfig rsFacet :=
  mkFacetJobConfig fun mod => do
    (← mod.leanArts.fetch).mapM (sync := true) fun arts => do
      let some art := arts.rs?
        | error "No Rust file generated. Ensure the Rust backend is enabled."
      /-
      Avoid recompiling unchanged Rust files.
      Rust files are assumed to only depend on their content
      and not transitively on their inputs (e.g., imports).
      -/
      newTrace s!"{mod.name.toString}:rs"
      addTrace art.trace
      return art.path
...
public def Module.oExportFacetConfig : ModuleFacetConfig oExportFacet :=
  mkFacetJobConfig (memoize := false) fun mod =>
    ...
    | .llvm => mod.bco.fetch
    | .rust => error "the Rust backend does not produce object files; use rustc to compile .rs files"
